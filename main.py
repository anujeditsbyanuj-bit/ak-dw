...
def plans_keyboard() -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton("📸 Send Payment Proof", url=POWERED_BY_URL)])
    rows.append([InlineKeyboardButton("⬅️ Back", callback_data="plans_back")])
...
def status_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📞 Contact Admin", url=POWERED_BY_URL)],
    ])
...
async def start_handler(client: Client, m: Message):
    ...
    try:
        await m.reply_photo(START_PHOTO_URL, caption=caption, reply_markup=MAIN_MENU_KB, parse_mode=ParseMode.HTML)
    except Exception as e:
        logger.warning(f"start photo failed, falling back to text: {e}")
        await m.reply(caption, reply_markup=MAIN_MENU_KB, parse_mode=ParseMode.HTML)
...
@app.on_message((filters.command("help") | filters.regex(r"^❓ Help$")) & filters.private)
async def help_handler(client: Client, m: Message):
    await m.reply(
        ...
        "https://diskwala.com/app/...\n"
        "https://thediskwala.com/app/...\n\n"
        "📌 <b>ᴇxᴀᴍᴘʟᴇ:</b>\n"
        "https://www.diskwala.com/app/6a996d5006ba7ea03db225c2\n\n"
        "💡 <b>ᴛɪᴘs:</b>\n"
        ...
        "• ᴜsᴇ /cancel ᴛᴏ sᴛᴏᴘ ᴀɴ ᴀᴄᴛɪᴠᴇ ᴅᴏᴡɴʟᴏᴀᴅ\n\n"
...
ABOUT_TEXT = (
    ...
    "<blockquote>"
    "┌─[ ✨ <b>ANUJ</b> ]────✪\n"
    "├✪ 🚀 <b>BOT NAME</b>  : <code>Most Powerful Src Bot</code>\n"
    "├✪ 🧑‍💻 <b>DEVELOPER</b>  : <code>Anuj Kumar</code>\n"
    "├✪ 🔗 <b>LIBRARY</b>  : <code>Pyrogram Async</code>\n"
    "├✪ ⚡ <b>LANGUAGE</b>  : <code>Python 3.11+</code>\n"
    "├✪ ⚙️ <b>DATABASE</b>  : <code>MongoDB</code>\n"
    "├✪ ⭐ <b>HOSTING</b>  : <code>Dedicated High-Speed VPS</code>\n"
    "└──────────✪"
    "</blockquote>"
)
...
def about_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("❌ Close", callback_data="about_close")]])


@app.on_message(filters.command("about") & filters.private)
async def about_cmd(client: Client, m: Message):
    await m.reply(ABOUT_TEXT, reply_markup=about_keyboard(), parse_mode=ParseMode.HTML)
...
def fallback_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📥 Download", callback_data="fallback_download"),
         InlineKeyboardButton("📊 Status", callback_data="fallback_status")],
    ])


@app.on_callback_query(filters.regex(r"^fallback_download$"))
async def fallback_download_cb(client: Client, query):
    await query.answer("👇 Apna Diskwala link bhejo boss!", show_alert=True)
...
async def view_thumb_cmd(client: Client, m: Message):
    ...
    try:
        ...
        await m.reply(f"❌ Error loading thumbnail: {e}\nPlease set a new one.")
...
async def del_thumb_cmd(client: Client, m: Message):
    thumb_id = await get_thumbnail(m.from_user.id)
    if not thumb_id:
        return await m.reply("ℹ️ You don't have a custom thumbnail set.")
...
async def thumb_mode_cmd(client: Client, m: Message):
    ...
    await m.reply(f"🖼 <b>Thumbnail Status</b>\n\n{status}\n{extra}", parse_mode=ParseMode.HTML)
...
async def setchat_cmd(client: Client, m: Message):
    ...
    if arg == "clear":
        ...
        return await m.reply("✅ <b>Dump Chat Cleared.</b>", parse_mode=ParseMode.HTML)
    try:
    ...
    try:
        ...
        return await m.reply(f"❌ <b>Unable to Access Chat</b>\n<i>{e}</i>", parse_mode=ParseMode.HTML)
...
async def admin_set_dump_cmd(client: Client, m: Message):
    if len(m.command) < 3:
        return await m.reply(
            "⚠️ <b>Usage:</b> <code>/set_dump &lt;user_id&gt; &lt;chat_id&gt;</code>\n"
    ...
    if arg == "clear":
        ...
        return await m.reply(f"✅ Dump chat cleared for <code>{target_id}</code>.", parse_mode=ParseMode.HTML)

    try:
        chat_id = int(m.command[2].strip())
    except ValueError:
        return await m.reply("⚠️ chat_id must be a number.")
    ...
    await m.reply(
        f"✅ Dump chat set for <code>{target_id}</code> → <code>{chat_id}</code> ({chat_title})",
        parse_mode=ParseMode.HTML,
    )
...
def settings_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📊 My Usage Stats", callback_data="settings_stats")],
        [InlineKeyboardButton("🗑 Dump Chat", callback_data="settings_dump")],
        [
...
async def settings_text(user_id: int) -> str:
    premium = await get_premium_status(user_id)
...
async def settings_cmd(client: Client, m: Message):
    await m.reply(await settings_text(m.from_user.id), reply_markup=settings_keyboard(), parse_mode=ParseMode.HTML)
...
async def settings_callbacks(client: Client, query):
    ...
    if data == "settings_stats":
        ...
        if thumb_id:
            ...
            await query.answer("Thumbnail preview sent below 👇")
            return
        else:
        ...
        await query.message.edit_text(
            await settings_text(user_id), reply_markup=settings_keyboard(), parse_mode=ParseMode.HTML
        )
...
def payment_keyboard(price: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ I've Paid", callback_data=f"paid_{price}")],
        [InlineKeyboardButton("📸 Send Payment Proof", url=POWERED_BY_URL)],
        [InlineKeyboardButton("⬅️ Back", callback_data="plans_back")],
    ])
...
async def paid_cb(client: Client, query):
    ...
    await query.answer("✅ Admin ko notify kar diya!", show_alert=True)
    await query.message.reply(
        "✅ Aapka payment claim admin ko bhej diya gaya hai.\n"
...
async def users_export_cmd(client: Client, m: Message):
    status = await m.reply("⏳ Gathering user data...")
    ...
    try:
        ...
        await status.edit_text(f"👥 <b>Total Users:</b> {len(export)}", parse_mode=ParseMode.HTML)
        await m.reply_document(tmp_path, caption=f"📄 {len(export)} users exported.")
    except Exception as e:
        await status.edit_text(f"⚠️ Error exporting users: {e}")
    finally:
        try:
            os.remove(tmp_path)
        except Exception:
...
async def set_channel_id_cmd(client: Client, m: Message):
    ...
    if not raw.startswith("-100") or not raw.lstrip("-").isdigit():
        return await m.reply(
            "⚠️ The ID must start with <code>-100</code>, e.g. <code>-1001234567890</code>.",
            parse_mode=ParseMode.HTML,
        )
    ...
    try:
        ...
        return await m.reply(
            f"⚠️ Couldn't verify that chat — make sure I've already been added there.\n<code>{str(e)[:300]}</code>",
            parse_mode=ParseMode.HTML,
        )
    ...
    await m.reply(f"✅ {note}: <b>{title}</b> (<code>{channel_id}</code>).", parse_mode=ParseMode.HTML)
...
async def channel_id_cmd(client: Client, m: Message):
    ...
    await m.reply("🔗 <b>Linked Backup Channels</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML)
...
async def del_channel_id_cmd(client: Client, m: Message):
    ...
    if not args:
        ...
        return await m.reply(f"🗑️ Unlinked all dynamically-added channels ({count} removed).")

    try:
        channel_id = int(args[0])
    except ValueError:
        return await m.reply(
            "⚠️ Channel ID must be a number, e.g. <code>-1001234567890</code>.",
            parse_mode=ParseMode.HTML,
        )

    removed = await remove_channel(channel_id)
    if removed:
        await m.reply(f"🗑️ Unlinked <code>{channel_id}</code>.", parse_mode=ParseMode.HTML)
    else:
        await m.reply(
            f"⚠️ <code>{channel_id}</code> wasn't linked (or it's set via config, not removable here).",
            parse_mode=ParseMode.HTML,
        )
...
@app.on_message(filters.private & ~filters.command(["start", "help", "myplan", "plans", "premium", "addpremium",
...
async def link_handler(client: Client, m: Message):
    ...
    if not links:
        return await m.reply(FALLBACK_TEXT, reply_markup=fallback_keyboard())
    ...
    try:
        while True:
            ...
            if now - last_edit_time >= 3.0:
                ...
                try:
                    await status_msg.edit_text(
                        f"🎞️ <b>Converting to {target_height}p...</b>\n"
                        f"<code>{name}</code>\n\n"
                        "╭━━━━❰Progress❱━➣\n"
                        f"┣⪼ [{progress_bar(pct)}]\n"
                        f"┣⪼ ✅ Done: {pct:.1f}%\n"
    ...
    async with download_semaphore:
        ...
        try:
            ...
            if quality != "auto":
                ...
                src_duration, _src_width, src_height = get_video_metadata(out_path)
                ...
                if src_height and target_height >= src_height:
                    ...
                    ok = await transcode_video_async(
                        out_path, transcoded_path, target_height,
                        total_duration=src_duration or 1,
                        status_msg=status_msg, name=name,
                    )
                    if ok:
                        try:
                            os.remove(out_path)
