...
def smallcaps_html(text):
    ...
    (<b>, <a href="...">, <blockquote>, etc.) and the contents of
    <code>...</code> completely untouched — so commands, UPI IDs, chat
    IDs, and {placeholder} examples the user needs to copy-paste stay
    exactly as typed, while everything else gets the small-caps look.
...
def link_menu_markup(link_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [make_button(SC("🔽 Download"), callback_data=f"menu|{link_id}", style=BTN_PRIMARY)],
...
def quality_menu_markup(link_id: str) -> InlineKeyboardMarkup:
    ...
    for label, value in QUALITY_OPTIONS:
        row.append(make_button(SC(label), callback_data=f"dlq|{link_id}|{value}", style=BTN_PRIMARY))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
...
ABOUT_TEXT = (
    ...
    f"├⍟ 👨‍💻 {smallcaps('Developer')}  : <a href=\"https://t.me/anujedits76\">{smallcaps('Anuj Kumar')}</a>\n"
...
async def link_handler(client: Client, m: Message):
    ...
    if not links:
        ...
        return await m.reply(SC(FALLBACK_TEXT), reply_markup=fallback_keyboard())
...
async def _download_video_inner(client: Client, query, link: str, quality: str, status_msg, chat_id: int):
    if True:
        ...
        try:
            ...
            downloaded = 0
            dl_quality_label = f"{quality}p" if quality != "auto" else "Auto (Best)"
            dl_tracker = ProgressTracker(status_msg, "Downloading", name, quality=dl_quality_label)
            ...
            if quality != "auto":
                ...
                await status_msg.edit_text(SC(f"<b>Checking source quality...</b>"), parse_mode=ParseMode.HTML)
                ...
                if src_height and target_height >= src_height:
                    ...
                    if ok:
                        ...
                        await status_msg.edit_text(
                            SC(f"<b>Conversion failed, uploading original quality instead...</b>"),
                            parse_mode=ParseMode.HTML,
                        )
                        quality_label = "Auto (Best)"
...
async def set_bot_commands_list():
    ...
    await app.set_bot_commands(commands[:100])
