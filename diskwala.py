...
def fetch_diskwala_video(link: str, auth: str) -> dict:
    ...
    logger.info(f"Calling API: {API_DOWNLOAD}")

    r = requests.post(API_DOWNLOAD, headers=headers, json={"link": link}, timeout=60)
    ...
    status_url = API_STATUS + "?link=" + quote(link, safe="")
