...
async def _get_clone_client(token: str, owner_id: int) -> Client:
    ...
    link_handler = _wiring["link_handler"]
    callback_handler = _wiring["callback_handler"]
    link_filter = _wiring["link_filter"]
    callback_filter = _wiring["callback_filter"]
    clone_start_text = _wiring["clone_start_text"]
    SC = _wiring["SC"]
    ...
    async def _clone_start(client, m: Message):
        await m.reply(SC(clone_start_text()), parse_mode=ParseMode.HTML)
    ...
    clone.add_handler(MessageHandler(link_handler, link_filter), group=0)
    clone.add_handler(CallbackQueryHandler(callback_handler, callback_filter), group=0)
    ...
    logger.info(f"Titanium: clone started for owner {owner_id} (token ...{token[-6:]})")
    return clone
...
def register_titanium_handlers(app, *, make_button, BTN_PRIMARY, BTN_DANGER, SC, ParseMode,
    ...
    def clone_start_text() -> str:
        main_username = get_username() or "the main bot"
        return (
            f"<b>⚡ This is your personal Titanium clone.</b>\n\n"
            f"Send a Diskwala/Flezen link here and it'll download exactly "
            f"like @{main_username}, just on this bot's own token. "
            f"/cancel stops an active download.\n\n"
            f"Only you can use this bot — everyone else is ignored."
        )
    ...
    _wiring.update(dict(
        ...
        clone_start_text=clone_start_text, SC=SC, ParseMode=ParseMode,
    ))
    ...
    async def _prompt_for_token(chat_id: int, owner_id: int):
    ...
    async def _titanium_addbot_cb(client, query: CallbackQuery):
        ...
        await _prompt_for_token(query.message.chat.id, owner_id)
    ...
    async def _titanium_autocreate_cb(client, query: CallbackQuery):
        ...
        suggested_username = f"titanium_{owner_id}_{int(time.time()) % 100000}bot"
        deep_link = (
            f"https://t.me/newbot/{me.username}/{suggested_username}"
            f"?name={quote('My Titanium Clone')}"
        )
    ...
    async def _handle_managed_bot_created(client, update, users, chats):
        """Raw-update handler — catches the messageActionManagedBotCreated
        service message Telegram sends after a user completes the
        auto-create flow above. Hooked at the raw level (not
        @app.on_message) because pyrogram's higher-level Message parser
        may not know how to build a friendly Message object around an
        action type this new, depending on the installed kurigram version.

        messageActionManagedBotCreated only carries the new bot's id — no
        correlation data back to the button that triggered it — but that's
        fine: this message is sent directly from the requesting user to
        this bot's own chat with them, so the message's sender IS the
        owner, with no extra bookkeeping needed."""
    ...
    async def _addbot_cmd(client, m: Message):
        ...
        if len(m.command) < 2:
            return await _prompt_for_token(m.chat.id, user_id)
