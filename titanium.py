...
async def _get_clone_client(token: str, owner_id: int) -> Client:
    ...
    menu_message_handlers = _wiring["menu_message_handlers"]
    menu_callback_handlers = _wiring["menu_callback_handlers"]
    ...
    # Same Plans / My Status / Help / Support experience as the main bot —
    # these reuse the main bot's own handler functions, not copies.
    for handler_fn, handler_filter in menu_message_handlers:
        clone.add_handler(MessageHandler(handler_fn, handler_filter & filters.private), group=-1)
    for handler_fn, handler_filter in menu_callback_handlers:
        clone.add_handler(CallbackQueryHandler(handler_fn, handler_filter), group=-1)
